...
3. 解压：在jexus安装包所在文件夹中，以root身份解压Jexus压缩包（安装包）。如：sudo tar -zxvf jexus-8.0.x-x64.tar.gz -C /usr
...
- 在“一个端口多个网站”的情况下，为了区分不同的网站，你必须对每个网站设置不同的域名或者使用不同的虚拟路径，这样，当用户访问时，Jexus才会知道将用户发过来的请求交给哪一个网站处理。
- 同一个端口，不能同时支持Http和Https两种不同的传输协议。
