...
一、Jexus 概述 [转到](#一jexus-概述)

二、Jexus 的安装与更新 [转到](#二jexus-的安装与更新)

三、Jexus 的操作命令 [转到](#三jexus-的操作命令)

四、Jexus 的全局配置 [转到](#四jexus-的全局配置)

五、Jexus 的网站配置 [转到](#五jexus-的网站配置)
...
（1）支持ASP.NET。在Linux上支持Asp.Net运行，是Jexus的重要的特色功能之一（甚至一度被部分用户误认为Jexus就是一款单纯的Asp.Net服务器）。Jexus直接支持
